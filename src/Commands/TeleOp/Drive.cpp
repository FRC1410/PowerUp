...
void Drive::Execute() {
	 ...
	 * Mode 3 - Baby Mode (Arcade Drive but only 4 directions)
	 */
	...
	if (driving_mode == 1) {
		...
		if (invert_driving == 1) {
			...
			x = -Robot::oi.ApplyExponent(Robot::oi.GetDriveAxis(tankRightAxis), exponent);
			y = -Robot::oi.ApplyExponent(Robot::oi.GetDriveAxis(tankLeftAxis), exponent);
		}
		...
		m = invert_driving * Robot::oi.ApplyExponent(m, exponent);
