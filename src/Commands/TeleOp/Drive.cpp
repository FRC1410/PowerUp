...
void Drive::Execute(/*double l, double r*/ /*double d, double m*/ /*double x, double y, double d, double m*/) {
	...
	//Single Stick
	/*m = sqrt(pow(Robot::oi.GetDriveAxis(0), 2) + pow(Robot::oi.GetDriveAxis(1), 2));
	if (m > 1) {
		x = -Robot::oi.GetDriveAxis(0)/m;
		y = Robot::oi.GetDriveAxis(1)/m;
	} else {
	...
	if (Robot::oi.GetDriverButton(6)) {
		...
		while (Robot::oi.GetDriverButton(6)) {}
	}
	if (singleStick == false) {
		Robot::drivetrain.TankDrive(Robot::oi.GetDriveAxis(tankLeftAxis), Robot::oi.GetDriveAxis(tankRightAxis));
	} else {
